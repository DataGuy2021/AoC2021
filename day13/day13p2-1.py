...
def main():
	...
	for j in fold_list:
		...
		if j[0] == 'y':
			...
			paper_arr = cut_array(reg_arr_y,  fold_arr_y, 0)
			#print('paper_arr_fold after y')
			#print(paper_arr_fold)
			#print()
		else:
			#fold x
			axis_fold_index_x = int(j[1])
			reg_arr_x = paper_arr[:, :axis_fold_index_x]
			fold_arr_x = paper_arr[:, axis_fold_index_x+1:]
...
def cut_array(start_arr, fold_arr, axis_name):
	...
	if start_arr_len > fold_arr_len:
		# add to fold_arr
		# add to end
		fold_arr = np.concatenate((add_arr, fold_arr), axis = axis_name)	
	elif fold_arr_len > start_arr_len:
		# cut fold 
		fold_arr = np.delete(fold_arr,slice(add_on), axis_name)
	# add arrays
	rtn_arr = start_arr + fold_arr
